...
def plot_anscombes_quartet():
    ...
    for line in axs:
        for ax in line:
            ax.scatter(df_anscombe[df_anscombe.group == i]
                       ['x'], df_anscombe[df_anscombe.group == i]['y'])
...
def display_widget():
    ...
    def dropdown_choice(value, plotted_stats, ax_text, sc):
        if value.new != plotted_stats:
            ax_text.clear()
            ax_text.axis('off')
        sc.set_offsets(df_datasaurus.groupby(
            'group').get_group(value.new)[['x', 'y']])
    ...
    dropdown_graph_1.observe(lambda value: dropdown_choice(
        value, plotted_stats_graph_1, ax_text_1, sc_1), names='value')
    statistics_graph_1.on_click(lambda value: get_stats(
        value, plotted_stats_graph_1, ax_text_1, dropdown_graph_1, 1))
    dropdown_graph_2.observe(lambda value: dropdown_choice(
        value, plotted_stats_graph_2, ax_text_2, sc_2), names='value')
    statistics_graph_2.on_click(lambda value: get_stats(
    ...
    for line in axs:
        for ax in line:
            if i > 12:
                ...
                ax.scatter(df_datasaurus[df_datasaurus.group == group]['x'],
                           df_datasaurus[df_datasaurus.group == group]['y'], s=4)
